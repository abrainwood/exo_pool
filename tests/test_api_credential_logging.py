...
async def test_full_login_auth_failure_leaves_no_raw_secret_in_last_auth_error(
    hass, entry, caplog
):
    session = _FakeSession(
        _FakeResponse(401, {"message": "Invalid credentials", "password": "hunter2"})
...
async def test_full_login_missing_id_token_does_not_log_the_partial_response(
    ...
    with caplog.at_level(logging.DEBUG), pytest.raises(
        Exception, match="No userPoolOAuth.IdToken received"
    ):
        await api._full_login(hass, entry, session)
...
async def test_full_login_missing_authentication_token_does_not_log_the_response(
    hass, entry, caplog
):
    session = _FakeSession(
        _FakeResponse(
    ...
    with caplog.at_level(logging.DEBUG), pytest.raises(
        Exception, match="No authentication_token received"
    ):
        await api._full_login(hass, entry, session)
...
async def test_refresh_token_missing_id_token_does_not_log_the_response(
    hass, entry, caplog
):
    session = _FakeSession(
        _FakeResponse(
