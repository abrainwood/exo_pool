...
def test_redact_hides_a_bare_secret_string_in_a_list_that_also_has_a_dict():
    hidden = redact.redact([{"password": "hunter2"}, "eyJ-bare-secret-token"])

    assert hidden[0]["password"] != "hunter2"
    assert hidden[1] != "eyJ-bare-secret-token"


def test_redact_accepts_a_mapping_proxy_without_being_wrapped_in_dict():
    from types import MappingProxyType

    hidden = redact.redact(
        MappingProxyType({"password": "hunter2", "serial_number": "JT00000000"})
    )
